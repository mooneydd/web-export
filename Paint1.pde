...
void setup()
{
  ...
  size(320 ,548);
  b1= new Button("Start/Stop", width-120, height-60, 100,50);
  b2 = new Button("Reset", 20, height-60, 100,50);
...
{  
  ...
  if(running){
    ...
    locX = (oldLocX + Fx*deltaT);
    locY = (oldLocY - Fy*deltaT);
    
     strokeWeight(25-int(2*sqrt(Fx*Fx + Fy*Fy)));
   w= int(sqrt(Fx*Fx + Fy*Fy));
  // println("w is " + w);
   
     line(int(oldLocX), int(oldLocY), int(locX), int(locY));
     
    }

//   println("oldlocX is "+ oldLocX);
//    println("oldlocY is "+ oldLocY);
//
//    println("locX is "+ locX);
//    println("locY is "+ locY);
//
//    println("Fx is "+ accel.getX());
//    println("Fy is "+ accel.getY());
//    println("Fz is "+ accel.getZ());
// 
}
